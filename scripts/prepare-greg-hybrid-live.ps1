...
if ($report.deployment -ne 'live' -or $runtimeStatus -ne 'ready' -or $firmware.status -ne 'ready') {
...
if ($PSCmdlet.ShouldProcess($configPath, 'Write device-specific live session configuration')) {
    ...
    $session | ConvertTo-Json -Depth 6 | Set-Content -LiteralPath $configPath -Encoding utf8NoBOM
}

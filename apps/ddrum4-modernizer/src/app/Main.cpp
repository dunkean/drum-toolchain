 ...
 public:
  ...
  bool saveAndReload(const juce::String& text) {
    ...
    try {
      ...
      if(!juce::File(profilePath_.string()).replaceWithText(text)) throw std::runtime_error("impossible d'écrire le profil");
  ...
  void start(const juce::String& inputId,const juce::String& outputId) {
    stop(); if(!profile_&&!runtimeProfile_){error_="Profil non chargé";return;} error_.clear();
    ...
    if(!outputOwned_){error_="Impossible d'ouvrir la sortie MIDI sélectionnée";return;}
    ...
    auto resolveRuntimeInput=[&](const std::string& requested)->const juce::MidiDeviceInfo* {
      ...
      if(matches.size()!=1) {
        error_=matches.empty() ? "Entrée runtime introuvable : "+juce::String(requested)
                               : "Entrée runtime ambiguë : "+juce::String(requested);
        return nullptr;
      }
    ...
    if(runtimeProfile_) {
      for(const auto& source:runtimeProfile_->sources) {
        ...
        if(!opened) { error_="Impossible d'ouvrir l'entrée runtime"; stop(); return; }
    ...
    if(inputs_.empty()){error_="Impossible d'ouvrir l'entrée MIDI sélectionnée";stop();return;}
...
class MainComponent final : public juce::Component, private juce::Timer {
 public:
  MainComponent() {
    ...
    runtimeStateLabel_.setText("Etat logique local au PC",juce::dontSendNotification); addAndMakeVisible(runtimeStateLabel_);
 ...
 private:
  ...
  void refresh(){const auto* p=router_.profile();const auto* c=router_.converter();const auto active=c?c->activeKit():0;for(size_t i=0;i<kits_.size();++i){juce::String label="—";if(p&&i<p->kits.size()){label=p->kits[i].label;for(const auto& binding:p->programs)if(binding.kitIndex==i){label+="  · PC "+juce::String(binding.program);break;}}kits_[i].setButtonText(label);kits_[i].onClick=[this,i]{if(auto*c=router_.converter())c->selectKit(i,"UI");};kits_[i].setEnabled(p&&i<p->kits.size());}status_.setText(router_.running()?"● CONNECTÉ":"● ARRÊTÉ",juce::dontSendNotification);start_.setButtonText(router_.running()?"Arrêter":"Démarrer"); juce::String detail=juce::String("Profil : ")+juce::String(router_.profilePath().string())+"\n"; if(router_.runtimeMode()) { refreshRuntimeControls(); const auto h=router_.runtimeHealth(); detail+="Renderer : "+router_.runtimeRenderer()+"\n"; detail+="Commandes Scene/VP de cette vue : locales au renderer PC; le Master Merger est requis pour les rendre globales.\n"; detail+=juce::String("Runtime : ")+juce::String((juce::int64)h.received)+" reçus, "+juce::String((juce::int64)h.rendered)+" rendus, echoes "+juce::String((juce::int64)h.echoes)+", drops "+juce::String(router_.queueDropped()); if(router_.runtimeSourceHash().isNotEmpty()) detail+="\nSource SHA-256: "+router_.runtimeSourceHash(); } else if(p&&c) { detail=juce::String("Kit actif : ")+juce::String(p->kits[active].label)+"  · origine : "+c->lastProgramOrigin()+"\n"+detail; } detail+="\n"; detail+=router_.error().isEmpty()?juce::String("Choisissez le ddrum4 en entrée et le port virtuel vu par SD3 / DrumGizmo."):router_.error(); info_.setText(detail,juce::dontSendNotification); }
